class SemiFinalRoundsController < ApplicationController
  ...
  def create
    @Round = SemiFinalRound.new(round_params)
    if @Round.blank?
      redirect_to new_player_semi_final_round_path
    else
      @Round.save
      redirect_to score_player_semi_final_rounds_path(@Round)
    end
  end
  ...
  def record
    @RRecord = SemiFinalRoundRecord.new(round_record_params)
    if @RRecord.blank?
      redirect_to score_player_semi_final_rounds_path
    else
    @RRecord.save
  ...
  def before_game
    ...
    @round = SemiFinalRound.find(params[:player_id])
    participation = Participation.all
    @participation = participation.pluck(:name)
    @points = Point.all

    # ポイントの算出
    team_point = SemiFinalRoundTeam.find_by(name: @round.opponent).base_point
