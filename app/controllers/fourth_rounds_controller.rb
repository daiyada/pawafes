class FourthRoundsController < ApplicationController
  before_action :before_game, only: [:score, :record, :catch_point]

  def new
    @player = Player.find(params[:player_id])
    team = FourthRoundTeam.all
    ...
    thirdround = ThirdRound.find(params[:player_id])
    @thirdclimate = thirdround.climate
  end

  def create
    @Round = FourthRound.new(round_params)
    if @Round.blank?
      redirect_to new_player_fourth_round_path
    else
      @Round.save
      redirect_to score_player_fourth_rounds_path(@Round)
    end
  end
  ...
  def record
    @RRecord = FourthRoundRecord.new(round_record_params)
    if @RRecord.blank?
      redirect_to score_player_fourth_rounds_path
    else
    @RRecord.save
  ...
  def before_game
    ...
    @round = FourthRound.find(params[:player_id])
    participation = Participation.all
    @participation = participation.pluck(:name)
    @points = Point.all

    # ポイントの算出
    team_point = FourthRoundTeam.find_by(name: @round.opponent).base_point
