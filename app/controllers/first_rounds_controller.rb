class FirstRoundsController < ApplicationController
  ...
  def create
    @Round = FirstRound.new(round_params)
    if @Round.blank?
      redirect_to new_player_first_round_path
    else
      @Round.save
      redirect_to score_player_first_rounds_path(@Round)
    end
  end
  ...
  def record
    @RRecord = FirstRoundRecord.new(round_record_params)
    if @RRecord.blank?
  ...
  def before_game
    ...
    # ポイントの算出
    team_point = FirstRoundTeam.find_by(name: @round.opponent).base_point
