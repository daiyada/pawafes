class SecondRoundsController < ApplicationController
  ...
  def create
    @Round = SecondRound.new(round_params)
    if @Round.blank?
      redirect_to new_player_second_round_path
    else
      @Round.save
      redirect_to score_player_second_rounds_path(@Round)
    end
  end
  ...
  def record
    ...
    if @RRecord.blank?
      redirect_to score_player_second_rounds_path
    else
    @RRecord.save
    difference = @RRecord.point_difference
      if difference > 0 
        ...
        redirect_to lose_player_third_rounds_path
      end
    end
  end
  ...
  def before_game
    ...
    @round = SecondRound.find(params[:player_id])
    participation = Participation.all
    @participation = participation.pluck(:name)
    @points = Point.all

    # ポイントの算出
    team_point = SecondRoundTeam.find_by(name: @round.opponent).base_point
